...
| **Today** | Calendar events from now through end of day, each with the assigned person's avatar + color. Events with a location in the next 2 h show "Leave in 25 min". Under them, in whatever room today leaves, **tomorrow's** events under a "Tomorrow · Tue" heading | Leave-by = event start − buffer (default 20 min). Refreshes every 5 min. Today is never shortened for tomorrow: the panel holds about 6 rows, tomorrow takes what is left (none when today fills it) and says "+2 more tomorrow" for the rest. The split follows the tablet's own clock, so tomorrow becomes today at midnight |
...
| **Empty states** | Wake window: "Log wake-up". Medicine zone hidden. Calendar: "Nothing else today" (with tomorrow's events still listed under it, which is the common evening case). Sticker chart: empty grid with a friendly illustration. Night Mode with no photos: dim clock only |
...
| The Today panel showed only the rest of today, so from late afternoon — when the next morning is what anyone at the tablet wants — it usually read "Nothing else today" and nothing else | It fills the room today leaves with **tomorrow's** events under their own heading ([7.2](#72-main-screen)). A display asks the events function for `days=2`; the server still answers one day to anything that doesn't ask, so an older tablet can never show tomorrow's events as today's. |
