...
function fakeApi(): Fake {
  return {
    ...
    connectIcs: vi.fn().mockResolvedValue({ connectionId: 'conn-2', selectionId: 'sel-2', name: 'Soccer' }),
...
describe('CalendarsSection', () => {
  ...
  describe('connect a calendar link', () => {
    it('connects, clears the link and reloads the list', async () => {
      const w = await mountSection()
  ...
  describe('Google and Microsoft', () => {
    it('disables a provider that isn’t set up yet', async () => {
      ...
      expect(w.get('[data-testid="connect-microsoft"]').attributes('disabled')).toBeUndefined()
    })
