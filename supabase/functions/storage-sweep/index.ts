 ...
 * from the private `household-photos` bucket through the Storage API, which (unlike SQL) removes the bytes.
 *
...
import { runSweep, type StoredObject, type SweepStore } from '../_shared/sweep.ts'
...
async function listAll(prefix: string): Promise<ListEntry[]> {
...
const handler = createSweepHandler({
  ...
  sweep: async (options, now) => {
    const report = await runSweep(store, options, now)
